...
class PublicIssueAvailableOutboxListenerTest {

    @Mock
    private OutboxMessageRecorder outboxMessageRecorder;

    @InjectMocks
    private PublicIssueAvailableOutboxListener listener;

    @Test
    @DisplayName("공개 이슈 생성 이벤트를 outbox 메시지 저장 요청으로 변환한다")
    void recordOutboxMessage() {
        ...
        assertThat(request.type()).isEqualTo(OutboxMessageType.PUBLIC_ISSUE_AVAILABLE);
        assertThat(request.aggregateId()).isEqualTo("10");
        assertThat(request.occurredAt()).isEqualTo(event.publicFeedCollectedAt());

        assertThat(request.payload()).isInstanceOf(PublicIssueAvailablePayload.class);
        PublicIssueAvailablePayload payload = (PublicIssueAvailablePayload) request.payload();
        assertThat(payload.issueId()).isEqualTo(event.issueId());
        assertThat(payload.newsletterId()).isEqualTo(event.newsletterId());
        assertThat(payload.category()).isEqualTo(event.category().name());
    ...
    void recordOutboxMessageBeforeCommit() throws Exception {
        Method method = PublicIssueAvailableOutboxListener.class.getDeclaredMethod(
                "recordOutboxMessage",
