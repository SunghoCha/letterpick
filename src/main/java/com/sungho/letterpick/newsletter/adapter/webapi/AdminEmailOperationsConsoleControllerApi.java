...
public interface AdminEmailOperationsConsoleControllerApi {
    ...
    @Operation(
            ...
            description = "관리자가 지정한 수신 시각 범위의 인입 메일 상태별 처리 건수를 조회한다. 범위를 지정하지 않으면 최근 24시간을 조회한다."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "403", description = "관리자 권한 없음")
    })
    EmailOperationsStatusSummaryResponse getStatusSummary(String receivedFrom, String receivedTo);

    @Operation(
            summary = "조치 필요 인입 메일 목록 조회",
            description = "관리자가 지정한 수신 시각 범위의 인입 메일 중 운영 조치가 필요한 메일을 최신순으로 조회한다. 범위를 지정하지 않으면 최근 24시간을 조회한다."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "403", description = "관리자 권한 없음")
    })
    EmailOperationsInboundEmailsResponse getActionRequiredItems(String receivedFrom, String receivedTo, Pageable pageable);
